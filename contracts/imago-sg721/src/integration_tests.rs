...
mod tests {
    ...
    use sg721_imago::ExecuteMsg as Sg721ExecuteMsg;
    ...
    pub fn minter_contract() -> Box<dyn Contract<StargazeMsgWrapper>> {
        let contract = ContractWrapper::new(
            imago_vending_minter::contract::execute,
            imago_vending_minter::contract::instantiate,
            imago_vending_minter::contract::query,
        )
        .with_reply(imago_vending_minter::contract::reply);
        Box::new(contract)
    }
